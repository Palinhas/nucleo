...
switch ($acao) {
	...
	break;
        ...
        $socio = $_POST['socio'];
        $result_socios = "SELECT * FROM socios WHERE socio='$socio'"; // verifica se o numero de sócio está inserido na tabela socios
        $resultado_socios = mysqli_query($conn, $result_socios);
        $row_socios = mysqli_fetch_array($resultado_socios);

          $nomev = $row_socios['nome'];
          $sociov = $row_socios['socio'];
