...
<div class="list-group" style="width: 200px;">
  ...
  <div class="col mb-3" style="flex: 50%; float: left;">
    <div class="mt-3"">
      <span class="d-block p-3 bg-success text-white">Consultar lista de sócios</span>
    </div><br>
<table id="tabela" class="table table-hover table-sm">
        <thead>
          ...
          <th scope="col">Ultima Quota</th>
          <th scope="col">Arquivo</th>
          <th scope="col">Opções</th>
            </tr>
        </thead>
<?php         
      $result_socios = "SELECT * FROM socios ORDER BY id DESC";
      $resultado_socios = mysqli_query($conn, $result_socios); 
      while ($row_socio = mysqli_fetch_assoc($resultado_socios)) 
      { 
        echo '
        <tr>
          <th scope="row">'.$row_socio["socio"].'</th>
          <td>'.$row_socio["nome"].'</td>
          <td>'.$row_socio["quota"].'</td>
          <td>'.$row_socio["id"].'</td>
          <td> 
            <a class="btn btn-outline-success mx-lg-2 btn-sm" href="consultar_socios.php?id='.$row_socio['id'].'" role="button"> CONSULTAR </a>
          </td>
        </tr>
        ';
      }// <a class="btn btn-outline-danger mx-lg-2 btn-sm" href="proc_apagar_socios.php?id='.$row_socio['id'].'" role="button"> APAGAR </a>
?>  
