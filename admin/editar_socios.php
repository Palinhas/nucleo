...
$result_socio = "SELECT * FROM socios WHERE id = '$id'"; 
$resultado_socio = mysqli_query($conn, $result_socio); 
$row_socio = mysqli_fetch_assoc($resultado_socio); 
 ?>
<!doctype html>
<html lang="pt">
  <head>
    ...
    <title>Editar Sócios | Site oficial do Núcleo do Sporting Clube de portugal de Campo Maior</title>
  </head>
  <body class="body"> 
...
<div class="list-group" style="width: 200px;">
  <a href="socios.php" class="list-group-item list-group-item-action list-group-item-success">Menú Sócios</a> 
  <a href="inserir_socios.php" class="list-group-item list-group-item-action">Inserir Sócios</a>
  ...
  <div class="col mb-3" style="flex: 50%; float: left; background: #eeeeee;">
    <div class="mt-3">
      <span class="d-block p-3 bg-success text-white">Adicionar sócios na base de dados</span>
    </div>
   <form method="POST" action="proc_edit_socio.php">
      <div class="form-group col-md-1">
        <label></label>         
        <input type="hidden" class="form-control" name="id" value="<?php echo $row_socio['id']; ?>"></input>
      </div>
      <div class="form-row mt-3">
        <div class="form-group col-md-3">
              <label>Número Sócio do núcleo</label>
             <input type="text" name="socio" class="form-control" placeholder="" value="<?php echo $row_socio['socio']; ?>">
        </div>
         <div class="form-group col-md-3">
              <label>Quota</label>
              <input type="date" name="quota" class="form-control" placeholder="" value="<?php echo $row_socio['quota']; ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-9">
              <label>Nome</label>
              <input type="text" name="nome" class="form-control" placeholder="" value="<?php echo $row_socio['nome']; ?>">
        </div>
        <div class="form-group col-md-3">
              <label>Dáta Nascimento</label>
              <input type="date" name="birth" class="form-control" placeholder="" value="<?php echo $row_socio['birth']; ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-12">
              <label>Morada</label>
              <input type="text" name="morada" class="form-control" placeholder="" value="<?php echo $row_socio['morada']; ?>">
        </div>
      </div>
        <div class="form-row">
        <div class="form-group col-md-3">
              <label>Codigo-postal</label>
              <input type="text" name="cp" class="form-control" placeholder="" value="<?php echo $row_socio['cp']; ?>">
        </div>
        <div class="form-group col-md-9">
              <label>Localidade</label>
              <input type="text" name="localidade" class="form-control" placeholder="" value="<?php echo $row_socio['localidade']; ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-4">
              <label>Número Cartão de cidadão</label>
              <input type="text" name="cc" class="form-control" placeholder="" value="<?php echo $row_socio['cc']; ?>">
        </div>
          <div class="form-group col-md-4">
              <label>Número Contribuinte</label>
              <input type="text" name="nif" class="form-control" placeholder="" value="<?php echo $row_socio['nif']; ?>">
        </div>
          <div class="form-group col-md-4">
              <label>Ocupação</label>
              <input type="text" name="ocupacao" class="form-control" placeholder="" value="<?php echo $row_socio['ocupacao']; ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-8">
              <label>E-mail</label>
              <input type="text" name="email" class="form-control" placeholder="" value="<?php echo $row_socio['email']; ?>">
        </div>
          <div class="form-group col-md-4">
              <label>Contacto telefónico</label>
              <input type="text" name="contacto" class="form-control" placeholder="" value="<?php echo $row_socio['contacto']; ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-6">
              <label>Número Sócio SCP</label>
              <input type="text" name="socio_scp" class="form-control" placeholder="" value="<?php echo $row_socio['socio_scp']; ?>">
        </div>
          <div class="form-group col-md-6">
              <label for="tipo_scp">Tipo de Sócio</label>
              <select name="tipo_scp" class="form-control" value="<?php echo $row_socio['tipo_scp']; ?>">
              <option selected><?php echo $row_socio['tipo_scp']; ?></option>
        ...
        <div class="col-auto">
            ...
            <a href="#" class="btn btn-sm btn-danger mb-2" data-toggle="modal" data-target="#cancelarModal">
        Cancelar
        </a>
                    <!-- Button validar modal -->
        <button type="button" class="btn btn-sm btn-success mb-2" data-toggle="modal" data-target="#registarModal">
          Validar
        </button>
      </div>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                Se pressionar no botão SIM efetua a modificação do cliente na base de dados, se pressionar o botão NÃO cancela o processo.
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-danger mb-2" data-dismiss="modal">NÃO</button>
                <button type="submit" class="btn btn-sm btn-success mb-2">SIM</button>
              </div>
            </div>
          </div>
        </div>              <!-- Modal Cancelar-->
        <div class="modal fade" id="cancelarModal" tabindex="-1" role="dialog" aria-labelledby="cancelarModal" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-danger mb-2" data-dismiss="modal">NÃO</button>
               <a class="btn btn-success btn-sm mb-2" role="button" href="socios.php">Sim</a>
              </div>
            </div>
          </div>
        </div>
